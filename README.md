...
| Azure 標準 neural | $15／百萬字元 | NT$24 | ✅ | 台灣中文音色最齊（曉臻、雲哲等），REST＋key header |
| Azure neural HD | $22／百萬字元（2026-03 降價） | NT$35 | ✅ | 最像真人的中文選項之一 |
| Google Cloud Neural2 | $16／百萬字元 | NT$25 | ✅ | **每月前 100 萬字元免費**＝月聽 20 本書不用錢 |
| Google Cloud Chirp 3 HD | $30／百萬字元 | NT$47 | ✅ | 同上享每月 100 萬字元免費額度 |
| Gemini 2.5 Flash TTS | $10／1M 音訊 token（25 token／秒 ≈ $0.9／小時） | NT$85–95 | ✅ | AI Studio key 可直連，有免費層（限流）；按時計費 |
...
- TTS provider 已抽象（`synthOpenAI()` / `speakBrowser()` 兩條路徑，由設定切換），要加 Azure／Google／Gemini／ElevenLabs 只需新增一個 `synthXxx()`，在 `getSegmentUrl()` 接上、設定面板加一組 key 欄位即可
