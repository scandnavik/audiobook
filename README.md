...
開 https://scandnavik.github.io/audiobook/ ，點「**語音設定**」展開，引擎選「**瀏覽器內建**」——這個免費、不用任何帳號。然後把一段文字貼進「**貼上文字**」分頁，按「**載入並切段**」，再按底部圓形的**播放鍵**，就會開始唸了。
...
1. **最快免費上手**：到 aistudio.google.com 按「Get API key」（Google 帳號登入即可、不用綁信用卡）→ 回到「語音設定」，引擎選「**Gemini**」、把 key 貼進欄位
