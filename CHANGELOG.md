...
### v1.3 — 手機鎖屏背景播放＋調速即時性（2026-06-12）

**雲端引擎：鎖屏關螢幕不再停播（核心架構）**
- 改用 A／B 雙 `<audio>` 緩衝：下一段預先載進待命元素，段落交界時零空隙直接 `play()`，音訊 session 不中斷。修「iOS Safari 螢幕關閉後播到段落交界就停」——根因是舊的單元素「`pause()` → 換 `src` → `play()`」空隙會讓系統收回背景音訊權限，下一段的程式化 `play()` 在背景被擋
- 維護 `mediaSession.playbackState`（playing／paused）與 `setPositionState`（時長／進度／速率），鎖屏與控制中心顯示更準
- 自動接續走待命元素交換，使用者跳段（上一段／下一段／章節）維持原路徑

**調速即時性**
- 雲端引擎：`playbackRate` 套到兩個音檔元素並設 `preservesPitch`，換速即時生效不變調
- 瀏覽器內建引擎：`speechSynthesis` 無法改「正在唸」語句的速度，改為調速後 debounce 400ms 用新速度重唸當前段（會跳回該段開頭，Web Speech API 限制）

**瀏覽器內建引擎背景限制提示**
- 手機上用瀏覽器內建語音時提示一次：鎖屏／切背景會被系統暫停（平台硬限），需要背景播放請改雲端引擎

**工程**
- `sw.js` 快取升 `audiobook-v3`

### v1.2 — 設定抽屜＋選項瘦身＋手機體驗修整（2026-06-10）

**設定隨時可調**
- 語音設定改為頂欄滑桿鈕開啟的覆蓋式抽屜，讀到書中任何位置都能調，不用捲回頁首

**選項瘦身（去重複）**
- 移除設定裡的「外觀」下拉（與頂欄主題鈕重複）
- 移除設定裡的「語速」滑桿：播放列 ＋／－ 成為唯一速度控制，語速一律走播放端（換速不重新合成、不重複收費、快取 key 更穩定）；Google／Azure／OpenAI 合成端語速參數一併拔除
- 「每段最長字數」預設改**自動配（依引擎）**：OpenAI mini-tts 1000／tts-1 4000／Google 1300／Azure 1800／Gemini 4000／瀏覽器 800，換引擎不用自己記數字；取消勾選可手動覆寫，書庫續聽仍沿用當時切段長度
...
### v1.1 — 播放列快速調速＋修「播放回到舊段落」（2026-06-10）

**快速調速**
- 調速併入播放控制列：「－」在上一段左側（減速）、「＋」在下一段右側（加速），每格 ±0.25×、範圍 0.5–3.0×，速度記住跨重開；目前速度顯示在上方資訊列（點一下回 1.0×），狀態膠囊同步移到資訊列
- 走播放端 `playbackRate` 疊加：雲端引擎換速立即生效、不重新合成、不重複收費；瀏覽器內建引擎下一段生效
