...
**重點＋筆記**
- 播放列新增「標記」鈕：一鍵標記現在唸到的句子（雲端引擎用播放時間比例推估＋句界吸附；瀏覽器引擎用 onboundary 精準定位）
- 段落文字改全文渲染，可直接選取畫重點（選字級），點高亮可寫筆記／刪除
- 新增「重點」分頁：依章節分組列出所有重點＋筆記，可匯出 Markdown（下載 .md 或複製剪貼簿），格式為 frontmatter＋章節分組＋引文 blockquote
- 重點存 IndexedDB 新 `marks` store（DB 升 v3），刪書連動刪重點
- 平台限制：鎖屏無法標記（mediaSession 不支援自訂鍵）

**整本匯出音檔**
- 段落清單與書庫列新增「匯出音檔」：按章節分檔（`01-章名.mp3`）打包 zip 下載；無章節書退化單檔直接下載
- 匯出前確認頁：預估費用（已快取字數標 $0）／檔案大小／合成時間；手機提示建議電腦端執行
- 批次佇列：併發 2、單段自動重試 2 次、可取消；取消或失敗已合成段留快取＝斷點續傳不重複收費
- Gemini 章級組裝：逐段 PCM 合併重包單一 WAV；瀏覽器內建引擎不支援匯出（無音訊資料）
- 手寫 ZIP writer（store 模式＋CRC32，UTF-8 檔名），與既有 EPUB ZIP parser round-trip 互驗

**工程**
- 新增 `tests/`：node --test 跑純函式測試（句界吸附／MD 產生／ZIP round-trip／WAV 合併／費用估算），從單檔 HTML 抽函式直測
- `getSegmentUrl()` 拆出 `getSegmentBlob()`，播放與匯出共用快取邏輯
- sw.js cache 升 `audiobook-v2`
